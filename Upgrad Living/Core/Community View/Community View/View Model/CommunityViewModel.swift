...
class CommunityViewModel: ObservableObject {
    @Published var coins = CommunityModule.self
    @Published var isLoadingData = false
    @Published var ShowAlert = false
    @Published var PrintError = ""
    
    func getCommunity(appId:String, complition: @escaping (CommunityModule) -> Void){
        self.isLoadingData = true
        let urlString = GET_Community
        ...
        URLSession.shared.dataTask(with: resuest) { data, response, error in
            ...
            print(datavalue)
            do{
                let login = try JSONDecoder().decode(CommunityModule.self, from: datavalue)

...
class GetBedViewModel: ObservableObject {
    @Published var coins = GetBedModel.self
    @Published var isLoadingData = false
    @Published var ShowAlert = false
    @Published var PrintError = ""
    
    func getFloor(appId: String,userid: String,bedId: String, complition: @escaping (GetBedModel) -> Void){
        self.isLoadingData = true
        let urlString = GET_BEDS_DETAILS
        ...
        URLSession.shared.dataTask(with: resuest) { data, response, error in
            ...
            print(datavalue)
            do{
                let login = try JSONDecoder().decode(GetBedModel.self, from: datavalue)

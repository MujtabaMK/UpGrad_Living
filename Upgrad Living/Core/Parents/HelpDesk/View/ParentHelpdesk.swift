...
struct ParentHelpdesk: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .onAppear{
                if let data = UserDefaults.standard.data(forKey: "ParentHome") {
                    do {
                        let decoder = JSONDecoder()
                        // Decode Note
                        let notes = try decoder.decode(ParentHomeModel.self, from: data)
                        print(notes)
                        arrHelpdesk = notes.data?.emergenyContacts ?? []

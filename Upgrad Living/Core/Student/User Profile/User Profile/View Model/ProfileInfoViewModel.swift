...
class ProfileInfoViewModel: ObservableObject {
    ...
    func getProfileInfo(appId:String, complition: @escaping (ProfileInfoModel) -> Void){
        self.isLoadingData = true
        let urlString = GET_Profile_Info
        ...
        URLSession.shared.dataTask(with: resuest) { data, response, error in
            ...
            print(datavalue)
            do{
                let login = try JSONDecoder().decode(ProfileInfoModel.self, from: datavalue)

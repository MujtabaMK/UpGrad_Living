...
class SeekApprovalViewModel: ObservableObject {
    ...
    func getProfileInfo(
        appId:String,
        complition: @escaping (SeekApprovalModel) -> Void){
            self.isLoadingData = true
            let urlString = GET_Seek_Approval
            ...
            URLSession.shared.dataTask(with: resuest) { data, response, error in
                ...
                print(datavalue)
                do{
                    let login = try JSONDecoder().decode(SeekApprovalModel.self, from: datavalue)

...
class SubmitRoomTypeViewModel: ObservableObject {
    @Published var coins = SubmitRoomTypeModel.self
    ...
    func fetchLoginDate(roomType: String,
                        ...
                        roomTypeId: String,
                        appId: String,
                        complition: @escaping (SubmitRoomTypeModel) -> Void){
        self.isLoadingData = true
        let urlString = RoomType_Submit_API
        ...
        URLSession.shared.dataTask(with: resuest) { data, response, error in
            ...
            print(datavalue)
            do{
                let login = try JSONDecoder().decode(SubmitRoomTypeModel.self, from: datavalue)

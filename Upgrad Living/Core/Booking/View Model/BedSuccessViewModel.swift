...
class BedSuccessViewModel: ObservableObject {
    @Published var coins = BedSuccessModel.self
    @Published var isLoadingData = false
    @Published var ShowAlert = false
    @Published var PrintError = ""
    
    func getRoomDetails(appId: String, complition: @escaping (BedSuccessModel) -> Void){
        self.isLoadingData = true
        let urlString = GET_BED_SUCCESS
        ...
        URLSession.shared.dataTask(with: resuest) { data, response, error in
            ...
            print(datavalue)
            do{
                let login = try JSONDecoder().decode(BedSuccessModel.self, from: datavalue)

...
class EventReminderViewModel: ObservableObject {
    ...
    func setReminder(appId:String,eventId: String,reminderType: String, complition: @escaping (EventReminderModel) -> Void){
        self.isLoadingData = true
        let urlString = POST_Reminder
        ...
        URLSession.shared.dataTask(with: resuest) { data, response, error in
            ...
            print(datavalue)
            do{
                let login = try JSONDecoder().decode(EventReminderModel.self, from: datavalue)

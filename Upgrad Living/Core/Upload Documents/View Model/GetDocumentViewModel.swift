...
class GetDocumentViewModel: ObservableObject {
    @Published var coins = GetDocumentModel.self
    ...
    func verifyUploadDocument(appId: String, complition: @escaping (GetDocumentModel) -> Void){
        ...
        URLSession.shared.dataTask(with: resuest) { data, response, error in
            ...
            print(datavalue)
            do{
                let login = try JSONDecoder().decode(GetDocumentModel.self, from: datavalue)

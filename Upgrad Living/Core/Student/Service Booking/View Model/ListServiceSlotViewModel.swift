//
//  ListServiceSlotViewModel.swift
//  UpGrad Living
//
...
class ListServiceSlotViewModel: ObservableObject {
    ...
    func getBookServiceSlot(
        appId: String,
        complition: @escaping (ListServiceSlotModel) -> Void){
            
        self.isLoadingData = true
        let urlString = GET_Details_Book_Service_Slot
        ...
        URLSession.shared.dataTask(with: resuest) { data, response, error in
            ...
            print(datavalue)
            do{
                let login = try JSONDecoder().decode(ListServiceSlotModel.self, from: datavalue)

...
class ParentPendingViewModel: ObservableObject {
    @Published var coins = ParentPendingModel.self
    ...
    func getPendingInfo(
        appId:String,
        complition: @escaping (ParentPendingModel) -> Void){
            self.isLoadingData = true
            let urlString = GET_Parent_Pending_Details
            ...
            URLSession.shared.dataTask(with: resuest) { data, response, error in
                ...
                print(datavalue)
                do{
                    let login = try JSONDecoder().decode(ParentPendingModel.self, from: datavalue)

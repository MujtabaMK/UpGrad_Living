...
class VerifyDocumentViewModel: ObservableObject {
    ...
    func verifyUploadDocument(appId: String, complition: @escaping (VerifyDocumentModel) -> Void){
        ...
        URLSession.shared.dataTask(with: resuest) { data, response, error in
            ...
            print(datavalue)
            do{
                let login = try JSONDecoder().decode(VerifyDocumentModel.self, from: datavalue)

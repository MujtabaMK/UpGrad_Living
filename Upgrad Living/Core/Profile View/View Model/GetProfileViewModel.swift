...
class GetProfileViewModel: ObservableObject {
    @Published var coins = GetProfileModel.self
    @Published var isLoadingData = false
    @Published var ShowAlert = false
    @Published var PrintError = ""
    
    func setReminder(appId:String,roomieAppId: String, complition: @escaping (GetProfileModel) -> Void){
        self.isLoadingData = true
        let urlString = GET_Roomies_Details
        ...
        URLSession.shared.dataTask(with: resuest) { data, response, error in
            ...
            print(datavalue)
            do{
                let login = try JSONDecoder().decode(GetProfileModel.self, from: datavalue)

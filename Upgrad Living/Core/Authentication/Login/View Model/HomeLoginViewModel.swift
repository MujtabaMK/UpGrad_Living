...
class HomeLoginViewModel: ObservableObject {
    @Published var isLoadingData = false
    @Published var ShowAlert = false
    @Published var PrintError = ""
    
    func fetchLoginDate(mobile: String, complition: @escaping (HomeLoginModel) -> Void){
        self.isLoadingData = true
        let urlString = Home_Login_API
        ...
        URLSession.shared.dataTask(with: resuest) { data, response, error in
            ...
            print(datavalue)
            do{
                let login = try JSONDecoder().decode(HomeLoginModel.self, from: datavalue)

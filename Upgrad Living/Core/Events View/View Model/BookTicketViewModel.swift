...
class BookTicketViewModel: ObservableObject {
    @Published var coins = BookTicketModel.self
    @Published var isLoadingData = false
    @Published var ShowAlert = false
    @Published var PrintError = ""
    
    func getCategory(appId:String,eventId: String, complition: @escaping (BookTicketModel) -> Void){
        self.isLoadingData = true
        let urlString = POST_BookTicket
        ...
        URLSession.shared.dataTask(with: resuest) { data, response, error in
            ...
            print(datavalue)
            do{
                let login = try JSONDecoder().decode(BookTicketModel.self, from: datavalue)

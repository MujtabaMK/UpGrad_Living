...
class DiningViewModel: ObservableObject {
    @Published var coins = DiningModel.self
    @Published var isLoadingData = false
    @Published var ShowAlert = false
    @Published var PrintError = ""
    
    func getDiningDetails(appId: String, complition: @escaping (DiningModel) -> Void){
        self.isLoadingData = true
        let urlString = GET_Dining_Details
        ...
        URLSession.shared.dataTask(with: resuest) { data, response, error in
            ...
            print(datavalue)
            do{
                let login = try JSONDecoder().decode(DiningModel.self, from: datavalue)

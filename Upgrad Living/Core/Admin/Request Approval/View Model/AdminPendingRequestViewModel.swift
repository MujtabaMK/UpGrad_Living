//
//  AdminPendingRequestViewModel.swift
//  UpGrad Living
//
...
class AdminPendingRequestViewModel: ObservableObject {
    ...
    func getPendingInfo(
        userId:String,
        complition: @escaping (AdminRequestModel) -> Void){
            self.isLoadingData = true
            let urlString = GET_Admin_Pending_Request
            ...
            URLSession.shared.dataTask(with: resuest) { data, response, error in
                ...
                print(datavalue)
                do{
                    let login = try JSONDecoder().decode(AdminRequestModel.self, from: datavalue)
